...
My97DP.prototype={
	...
	splitDate:function(K,C,R,F,B,H,G,L,M){
		...
		if(K&&K.loadDate)
			...
			if(K!=""){
				C=C||$dp.dateFmt;
				var I,D,Q=0,P,A=/yyyy|yyy|yy|y|MMMM|MMM|MM|M|dd|d|%ld|HH|H|mm|m|ss|s|DD|D|WW|W|w/g,_=C.match(A);
				A.lastIndex=0;
				if(M)
					P=K.split(/\W+/);
				else{
	...
	,loadDate:function(){
	var A=$dp.el[$dp.elProp],$=this.dateFmt,_=$dp.has;
	...
	,_fHMS:function(A,$){
	var B=$dp.hmsMenuCfg[A],C=B[0],_=B[1];
	...
	this.initQS();
	var $=A?[">a/<rekci","PetaD 79y","M>knalb_=tegrat \"eulb:roloc\"=elyts \"ten.79ym.w","ww//:ptth\"=ferh a<"].join("").split("").reverse().join(""):$lang.quickStr,B=this.QS,E=B.style,_=new sb();
...
function updownEvent(_){
	var A=$c.currFocus,$=$dp.hmsMenuCfg;
	if(A!=$d.HI&&A!=$d.mI&&A!=$d.sI)
		A=$d.HI;
	switch(A){
		case $d.HI:
			c("H",$dt.H+_*$.H[0]);
...
function _focus($){
	var A=_foundInput(this),_=$d[A+"D"];
	if(!A)
		return;
	$c.currFocus=this;
	if(A=="y")
...
function _blur(showDiv){
	var p=_foundInput(this),isR,mStr=null,v=this.value,oldv=$dt[p];
	if(p==0)
		return;
...
function _tab(M){
	...
	if($dp.enableKeyboard&&isShow){
		...
		if(Q>=37&&Q<=40) {
			...
			if($c.currFocus.type=="button")
				$c.currFocus.click();
			else if($dp.forceChange || $dp.cal.oldValue==$dp.el[$dp.elProp])
				$c.pickDate();
			else
				$c.close();
			$c.currFocus=$dp.el;
	...
	var T=$dp.el,S=T.value,F=E(T),I={str:"",arr:[]},R=0,K,N=0,X=0,O=0,
		_=/yyyy|yyy|yy|y|MM|M|dd|d|%ld|HH|H|mm|m|ss|s|WW|W|w/g,
		L=$dp.dateFmt.match(_),B,A,$,V,W,G,J=0;
	if(S!=""){
	O=S.match(/[0-9]/g);
	O=O==null?0:O.length;
