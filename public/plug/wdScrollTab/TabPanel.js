...
TabPanel = function(config){
  ...
  this.maxzindex = 0;
  this.activeTabId = 0;//当前选中tab
  this.preTabId = 0;//上一个选中tab
  this.init();
};
...
TabPanel.prototype = {
  ...
  init : function(){
	...
	this.tabpanel_left_scroll.append('<span class="icon-arrow-left"></span>');
	...
	this.tabpanel_right_scroll.append('<span class="icon-arrow-right"></span>');
  ...
  moveToSee : function(position){
    if(this.scrolled)
    {
	  ...
	  var liNext = this.tabWidth + liWhere;
      var ulWhere = parseInt(this.tabpanel_mover.css('marginLeft'));
	  var ulWidth = this.tabpanel_move_content.outerWidth();
	  //如果已经在显示区域，就不调整
	  if((liWhere+ulWhere) >= 0 && (liNext < (ulWidth-ulWhere)) ) {
		  return;
	  }
      var moveNum;
      if(ulWhere<=0)
  ...
  update : function(){
    ...
    if(this.scrolled) {
      cWidth -= (this.tabpanel_left_scroll.outerWidth()+this.tabpanel_right_scroll.outerWidth());
    }
  ...
  show : function(position, notExecuteMoveSee){
	...
	if(this.activeTabId != position) {
		this.preTabId = this.activeTabId;
		this.activeTabId = position;
	}
    if(this.scrollFinish)
    {
	  ...
	  //this.tabpanel_content.children().hide();
	  //this.tabs[position].content.show();
	  this.tabpanel_content.children().css('left', -5000);
	  this.tabs[position].content.css('left', 0);
  ...
  kill : function(position){
	$.onecloud.showMess("关闭", "succ", "close");
    var tabEntity = this;
    //get tab index
    position = this.getTabPosision(position);
    ...
    this.showScroll();
	//删除后，后面位置往前移动一格
	if(this.preTabId >= position) {
		this.preTabId --;
	}
	if(this.activeTabId == position) {//关闭当前打开的
		this.show(this.preTabId, false);
	} else {
		if(this.activeTabId > position) {
			this.activeTabId --;
		}
		this.show(this.activeTabId, false);
	}
  },
  ...
  setTitle : function(position,title){
    position = this.getTabPosision(position);
    if(position < this.tabs.length)
      this.tabs[position].title.text(title).attr('title', title);
  },
