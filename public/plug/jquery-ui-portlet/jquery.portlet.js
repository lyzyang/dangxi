...
(function($, undefined){
    $.widget("ui.portlet", {
        ...
        _create: function() {
			...
			var fixedWidth = 0;
			var changeIndex = [];
			$.each(o.columns, function(k, c) {
				if(c.proportion) {//使用百分比
					changeIndex.push(k);
					if(! c.minWidth) {
						c.minWidth = c.width;
					}
			...
			fixedWidth += parseInt(_ele.css('padding-left'));
			var resizeFun = function() {
				if(_ele.children().length==0) {
					$(window).unbind('resize', resizeFun);
					return;
				}
				var containerWidth = _ele.parent().width();
				$.each(changeIndex, function(k, v) {
					var columnOption = o.columns[v];
					var columnWidth = (containerWidth - fixedWidth) * columnOption.proportion;
					if(columnWidth < columnOption.minWidth) {
						columnWidth = columnOption.minWidth;
					}
					o.columns[v].width = columnWidth;
					_ele.children("div:eq(" + v + ")").width(columnWidth);
				});
				//设置主窗口的宽度，防止小窗口位置不够时换行
				var contentWidth = 0;
				$.each(o.columns, function(k, c) {
					contentWidth += c.width;
				});
				_ele.width(contentWidth);
			};
            ...
            $(_ele).find('.ui-portlet-header').dblclick(function() {
                ...
                if($item.hasClass('ui-portlet-single-view')) {
                    ...
                    $item.removeClass('ui-portlet-single-view').animate({
                        ...
                        height: $item.data('height') //'auto' // important
                    }).css({
                        position: 'static'
        ...
        _sortable: function(value) {
            ...
            var st = $(".ui-portlet-column", this.element).sortable({
				...
				handle: ".ui-portlet-header",//@update
        ...
        _initEvents: function(element) {
            ...
            var toggle = $(".ui-portlet-toggle", _ele).click(function(event, type) {
                ...
                type = type || 'toggle';
                if (type == 'toggle') {
                    ct.slideToggle(function() {$(window).trigger('resize');});//@update
                    $(this).toggleClass("ui-icon-minusthick").toggleClass("ui-icon-plusthick");
                } else if (type == 'hide') {
                    ct.slideUp(function() {$(window).trigger('resize');});//@update
                    $(this).removeClass("ui-icon-minusthick").addClass("ui-icon-plusthick");
                } else if (type == 'show') {
                    ct.slideDown(function() {$(window).trigger('resize');});//@update
